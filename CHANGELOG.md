...
- CMR-managed jobs can now turn on 'Limit sowing to computed need': instead of sowing every eligible cell as soon as the active crop's hysteresis allows sowing, it estimates how many cells are needed to close the gap to the crop's upper bound and only sows that many (favoring the most fertile cells first), inflated by an adjustable safety multiplier (a slider from 50% to 200%, in 10% steps, defaulting to 120%). Any cells left over aren't left idle - they're sown with the next crop in the rotation instead, cascading further down the rotation if that crop's own need is covered too. This is only ever an estimate - it can't account for blights, fires, cold snaps, or anything else the game throws at a growing crop before harvest - so it's off by default. If the job's switch mode is set to 'Switch immediately', cells sown ahead of need this way get cut down before they mature once their own turn in the rotation comes up, same as any other leftover - a warning explains this when both are on together. While this is on, each crop in the rotation list now also shows its estimated yield per cell, how many cells it's estimated to need, and how many cells are currently assigned to it.
