...
- **Crop rotation**: set up an ordered list of crops for a job, and once the current crop's stock threshold is met, growers automatically move on to the next crop in the list, cycling back around at the end. The last crop in the list can instead be marked 'Grow indefinitely', so it keeps growing forever once every earlier crop's own need is covered, rather than stopping once it reaches its own upper bound.
- **Limit sowing to computed need**: instead of sowing every eligible cell as soon as a crop's hysteresis allows it, estimate how many cells are actually needed to close the gap to its upper bound and only sow that many, favoring the most fertile cells first. Any cells left over cascade to the next crop in the rotation instead of sitting idle. This is only ever an estimate — it can't account for blights, fires, or anything else the game throws at a growing crop before harvest — so it's off by default.
